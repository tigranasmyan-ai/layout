...
export default function SpacingOverlay({ activeShape, camera, onUpdate, editor }) {
    ...
    useEffect(() => {
        const handleDblClick = (e) => {
            const marginX = 12; // detection radius
            const marginY = 12;
            
            // Get coordinates relative to the Tldraw canvas container
            ...
            const isNearLeft = Math.abs(mx - leftEdge) <= marginX && my >= topEdge && my <= bottomEdge;
            const isNearRight = Math.abs(mx - rightEdge) <= marginX && my >= topEdge && my <= bottomEdge;
            const isNearTop = Math.abs(my - topEdge) <= marginY && mx >= leftEdge && mx <= rightEdge;
            const isNearBottom = Math.abs(my - bottomEdge) <= marginY && mx >= leftEdge && mx <= rightEdge;
            ...
            if (isNearLeft || isNearRight || isNearTop || isNearBottom) {
                ...
                if (isNearLeft || isNearRight) {
                    if (e.altKey) {
                        const restoreW = meta.baseW || 100;
                        onUpdate({ isFullW: false, isGrow: false, isAutoW: true }, activeShape.id);
                        // Force shape update to restore physical width
                        if (editor) {
                            window.__lastLayoutEngineUpdate = Date.now();
                            editor.updateShape({ id: activeShape.id, props: { w: restoreW } });
                        }
                    } else {
                        const baseW = (meta.isFullW || meta.isGrow) ? meta.baseW : activeShape.props.w;
                        onUpdate({ isFullW: true, isAutoW: false, baseW }, activeShape.id);
                    }
                } else {
                    if (e.altKey) {
                        const restoreH = meta.baseH || 100;
                        onUpdate({ isFullH: false, isAutoH: true }, activeShape.id);
                        if (editor) {
                            window.__lastLayoutEngineUpdate = Date.now();
                            editor.updateShape({ id: activeShape.id, props: { h: restoreH } });
                        }
                    } else {
                        const baseH = meta.isFullH ? meta.baseH : activeShape.props.h;
                        onUpdate({ isFullH: true, isAutoH: false, baseH }, activeShape.id);
                    }
                }
            }
        };
    ...
    }, [x, y, w, h, activeShape.id, onUpdate]);
